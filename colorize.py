...
def main(args):
    ...
    def visualize(model, data):
        ...
        output_dir = './colored_output'
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        for i in range(batch_size):
            for t in range(temporal_dim):
                fake_img = lab_to_rgb(L[i, t].unsqueeze(0), fake_color[i, t].unsqueeze(0))
                real_img = lab_to_rgb(L[i, t].unsqueeze(0), real_color[i, t].unsqueeze(0))

                fake_img_path = os.path.join(output_dir, f"fake_image_{i}_{t}_{time.time()}.png")
                real_img_path = os.path.join(output_dir, f"real_image_{i}_{t}_{time.time()}.png")

                plt.imsave(fake_img_path, np.squeeze(fake_img))
                
                print(f"Fake image saved at: {fake_img_path}")
...
if __name__=="__main__":
    ...
    parser.add_argument('--tempo_length', type=int, default=10)
