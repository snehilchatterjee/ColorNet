...
class DynamicUnet(SequentialEx):
    ...
    def __init__(self, encoder, n_out=2, img_size=(256,256), lstm=False, blur=False, blur_final=True, self_attention=False,
                 y_range=None, last_cross=True, bottle=False, act_cls=torch.nn.modules.activation.ReLU,
        ...
        ni = sizes[-1][1]
        middle_conv1 = ConvLayer(ni, ni*2, act_cls=act_cls, norm_type=norm_type, **kwargs).eval()

        middle_conv2 = ConvLayer(ni*2, ni, act_cls=act_cls, norm_type=norm_type, **kwargs).eval()
        
        x = middle_conv2(middle_conv1(x))
        
        #print(x.shape)
        layers = [encoder, BatchNorm(ni), nn.ReLU(), middle_conv1,middle_conv2]
